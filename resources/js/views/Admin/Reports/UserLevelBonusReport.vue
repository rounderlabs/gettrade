<template>
    <MainAdminLayout>
        <div class="card">
            <h5 class="card-title mb-2">User Level Bonus Report</h5>
            ...
            <div class="card-body table-responsive p-0">
                <table class="table table-striped text-nowrap">
                    ...
                    <tr v-else-if="!levelData.data.length">
                        <td colspan="6" class="text-center py-3">No records found</td>
                    </tr>
                    <tr v-for="(row, index) in levelData.data" :key="row.id">
                        <td>{{ index + 1 + ((levelData.current_page - 1) * levelData.per_page) }}</td>
                        <td>{{ row.user?.username || '-' }}</td>
                        <td>{{ row.subscription.user.username }}</td>
                        <td>{{ row.level }}</td>
                        <td>{{ formatCurrency(row.income_usd) }}</td>
                        <td>{{ formatCurrency(row.income_percent) }}</td>
...
const levelData = ref({ data: [] });
const loading = ref(false);
const filters = ref({ search: "", from: "", to: "" });

const fetchData = async () => {
    loading.value = true;
    const res = await axios.get("/admin/reports/get-user-level-bonus-report", { params: filters.value });
...
const exportCsv = () => {
    ...
    window.open(`/admin/reports/user-level-bonus/export?${query}`, "_blank");
};
</script>
